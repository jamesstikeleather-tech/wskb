...
class _ProcessEditorPageState extends State<ProcessEditorPage> {
  ...
  late TextEditingController _timeEstimate;
  ...
  void initState() {
    ...
    _timeEstimate = TextEditingController(
      text: p?.timeEstimateMin == null ? '' : p!.timeEstimateMin.toString(),
    );
  ...
  void dispose() {
    ...
    _tags.dispose();
    _useCases.dispose();
    _timeEstimate.dispose();
    super.dispose();
  }

  void _addPass() {
    final nextOrder = _passes.isEmpty
        ? 1
        : (_passes.map((e) => e.order).reduce((a, b) => a > b ? a : b) + 1);
    setState(() {
  ...
  void _save() {
    ...
    final updated = PersonalProcess(
      ...
      ownerUserId: widget.initial?.ownerUserId ?? 'local-demo',
      name: _name.text.trim(),
      description:
          _description.text.trim().isEmpty ? null : _description.text.trim(),
      tags: _splitCsv(_tags.text),
      useCases: _splitCsv(_useCases.text),
      timeEstimateMin: _parseInt(_timeEstimate.text),
      isActive: _isActive,
      isFavorite: _isFavorite,
      defaults: widget.initial?.defaults ?? const ProcessDefaults(),
      passPlan: (_passes..sort((a, b) => a.order.compareTo(b.order))),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            Text(widget.initial == null ? 'New Process' : 'Edit Process'),
        actions: [
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          TextField(
            controller: _timeEstimate,
            keyboardType: TextInputType.number,
            decoration: const InputDecoration(
              labelText: 'Time estimate (min)',
            ),
          ),
          const SizedBox(height: 8),
          SwitchListTile(
              ...
              setState(() {
                ...
                for (var i = 0; i < _passes.length; i++) {
                  ...
                  _passes[i] = PassPlan(
                    ...
                    areas: src.areas,
                    razorSetting: src.razorSetting,
                    angleCue: src.angleCue,
                    pressureCue: src.pressureCue,
                  );
                }
              });
            },
                  ...
                  child: Column(
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration:
                            const InputDecoration(labelText: 'Areas (optional)'),
                        onChanged: (v) {
                          setState(() {
                            _passes[i] = PassPlan(
                              order: p.order,
                              ...
                              setState(() {
                                _passes[i] = PassPlan(
                                  ...
                                  razorSetting: RazorSetting(
                                    plate: v.isEmpty ? null : v,
                                    adjustableSetting:
                                        p.razorSetting.adjustableSetting,
                                  ),
                                  angleCue: p.angleCue,
                                  pressureCue: p.pressureCue,
                                );
                      ...
                      TextFormField(
                        ...
                        decoration: const InputDecoration(
                            labelText: 'Pressure cue (optional)'),
                        onChanged: (v) {
                          setState(() {
                            _passes[i] = PassPlan(
                              order: p.order,
